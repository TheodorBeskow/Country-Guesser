...
with open('annotation_file_data2.json', 'r') as f:
    data = json.load(f)
...
for i, filename in enumerate(os.listdir(src_dir)):
    ...
    if name + '.json' in data:
        ...
        if data[name + '.json']:
            img_resized.save(os.path.join(dst_dir_true, filename), 'JPEG', quality=20)
        else:
            img_resized.save(os.path.join(dst_dir_false, filename), 'JPEG', quality=20)
